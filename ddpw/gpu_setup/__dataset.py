...
def dataset_setup(rank: int, p_config: PlatformConfig,
  ...
  if (train_set := artefacts.train_set) is not None:
    ...
    if artefacts.needs_validation:
      ...
      val_set = sampler(val_set, p_config.world_size, rank, batch_size, is_cpu)

    train_set = sampler(train_set, p_config.world_size, rank, batch_size,
                        is_cpu)
  ...
  if (test_set := artefacts.test_set) is not None:
    ...
    test_set = sampler(test_set, p_config.world_size, rank, batch_size, is_cpu)
